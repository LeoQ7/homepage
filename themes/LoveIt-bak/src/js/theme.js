...
class Theme {
    ...
    initSwitchTheme() {
        const mediaChangeListener = (e, newIsDark) => {
            if (e.matches && localStorage.getItem('theme') === null) {
                document.body.setAttribute('theme', newIsDark ? 'dark' : 'light');
        ...
        window.matchMedia("(prefers-color-scheme:dark)").addListener(e => mediaChangeListener(e, true));
        window.matchMedia("(prefers-color-scheme:light)").addListener(e => mediaChangeListener(e, false));

        const changeIcon = (iconParent, nextOption) => {
            const iconClassMap = {
                'dark': 'fa-moon',
                'light': 'fa-sun',
                null: 'fa-adjust',
            };
            iconParent.setAttribute('theme-button', nextOption || 'auto');
            this.util.forEach(iconParent.getElementsByTagName('i'), icon => {
                icon.classList.remove('fa-adjust', 'fa-sun', 'fa-moon');
                icon.classList.add(iconClassMap[nextOption]);
            });
        };

        this.util.forEach(document.getElementsByClassName('theme-switch'), $themeSwitch => {
            changeIcon($themeSwitch, localStorage.getItem('theme'));

            $themeSwitch.addEventListener('click', () => {
                // State machine:
                // media dark:  auto -> light -> dark  -> auto
                // media light: auto -> dark  -> light -> auto
                // neither:     auto -> dark  -> light -> auto

                const mediaIsDark = window.matchMedia('(prefers-color-scheme:dark)').matches;

                // null represents auto
                const lastOption = localStorage.getItem('theme');
                const lastName   = document.body.getAttribute('theme') || 'light';
                let   nextOption = null;

                switch (lastOption) {
                    case 'light':
                        nextOption = mediaIsDark ? 'dark' : null; break;
                    case 'dark':
                        nextOption = mediaIsDark ? null : 'light'; break;
                    case null: default:
                        nextOption = mediaIsDark ? 'light' : 'dark';
                }

                const nextName = nextOption || (mediaIsDark ? 'dark' : 'light');

                nextOption === null ? localStorage.removeItem('theme') : localStorage.setItem('theme', nextOption);

                document.body.setAttribute('theme', nextName);

                this.util.forEach(document.getElementsByClassName('theme-switch'), ele => changeIcon(ele, nextOption));

                this.isDark = nextName === 'dark';
